...
PerformTransactionDAO DAO;
	...
	public UserDetails performTransaction( long acc,TransactionDetails trans1) {
		...
		{
			throw new BankManagementException("transactionAmount:AccountBalance should be greater than TransactionAmount");
		}
		...
		if(ud.getAccountType().equals("savings"))
		{
			...
			{
				throw new BankManagementException("transactionType:Minimum balance in savings account should be greater than 5000");
			}
		...
		String q=Double.toString((trans1.getTransactionAmount()));
		if(trans1.getTransactionAmount()<=0 || (q.matches("/^[ A-Za-z_@./#&+-]*$/") || q==null || q.trim().equals("")))
		{
			throw new BankManagementException("transactionAmount:Enter valid Amount");
		}
		
		if(trans1.getTransactionDescription()==null || trans1.getTransactionDescription().trim().equals(""))
			
		{
	throw new BankManagementException("transactionDescription:Please enter valid name");

		}
		
		UserDetails ud1=DAO.performTransaction(acc,trans1);
		return ud1;
	...
	public void addSingleUser(UserDetails user1) {
		...
		if(user1.getAccountType().equals("Savings"))
		{
			...
			if(user1.getAccountType().equals("Salary"))
			{
				...
				{
					throw new BankManagementException("transactionType:Minimum  balance in salary account should be 0");
				}
			}
